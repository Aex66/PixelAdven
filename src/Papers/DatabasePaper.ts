...
import { asciiToText, textToAscii } from './Paragraphs/ConvertersParagraphs';
const release = 5
/*
...
class DatabasePaper {
    ...
    async registry(file: string): Promise<registry> {
        try { await world.getDimension('overworld').runCommand(`scoreboard objectives add "${file}" dummy`); } catch { }
     ...
     * @returns {database}
     */
    register(table: string, identifier?: string): database {
        ...
        world.scoreboard.getObjective(`DB:model`).setScore(identifier, 0)
        try {
            world.scoreboard.addObjective(`DB:${identifier}`)
            world.scoreboard.addObjective(`${identifier}:${table}`)
        } catch { };
        world.scoreboard.getObjective(`DB:${identifier}`).setScore(table, 0)
    ...
    drop(table: string, identifier?: string): void {
        if (!identifier) identifier = '';
        world.getDimension('overworld').runCommand(`scoreboard players reset "${table}" "DB:${identifier}"`);
        try { world.scoreboard.removeObjective(`${identifier}:${table}`); } catch { };
        if (this.allTables(identifier).length === 1) {
            world.getDimension('overworld').runCommand(`scoreboard players reset "${identifier}" "DB:model"`);
...
class registry {
    ...
    write(key: string | number, value: number): this {
        ...
        world.scoreboard.getObjective(this.file).setScore(String(key), value)
        return this;
    }
    /**
    ...
    writeMany(data: { [key: string | number]: number }): this {
        Object.keys(data).forEach(key => {
            ...
            world.scoreboard.getObjective(this.file).setScore(String(key), data[key])
        });
        return this;
    }
    /**
     ...
     * @returns {this}
     */
    shift(key: string | number, value?: number): this {//@ts-ignore
        regMemory[this.file][0][key] = regMemory[this.file][0][key] + value ?? 1;
        world.scoreboard.getObjective(this.file).addScore(String(key), value)
        return this;
    }
    /**
    ...
    delete(key: string | number): this {
        ...
        try { world.getDimension('overworld').runCommand(`scoreboard players reset "${key}" "${this.file}"`); } catch { }
        return this;
    }
    /**
    ...
    deleteMany(keys: string[]): this {
        for (const k of keys) {
            ...
            try { world.getDimension('overworld').runCommand(`scoreboard players reset "${k}" "${this.file}"`); } catch { }
        }
        return this;
    }
    /**
    ...
    clear(): this {
        ...
        try { world.getDimension('overworld').runCommand(`scoreboard objectives remove "${this.file}" dummy`) } catch { };
        try { world.getDimension('overworld').runCommand(`scoreboard objectives add "${this.file}" dummy`) } catch { };
        return this;
    }
    /**
...
class database {
    ...
    write(key: string, value: any): this {
        ...
        if (valueL > data.length) for (let l = 1; l < valueL; l++) world.scoreboard.getObjective(this.fullName).removeParticipant(`${key}=${l}`)
        for (const hex of data) world.scoreboard.getObjective(this.fullName).setScore(`${key}=${j}`, hex), j++;
        world.scoreboard.getObjective(this.fullName).setScore(key, 0)
        return this;
    }
    /**
    ...
    writeMany(data: { [key: string]: any }): this {
        ...
        for (const k of keys) {
            ...
            if (valueL > value.length) for (let l = 1; l < valueL; l++) try { world.getDimension('overworld').runCommand(`scoreboard players reset "${k}=${l}" "${this.fullName}"`); } catch { }
            for (const hex of value) world.scoreboard.getObjective(this.fullName).setScore(`${k}=${j}`, hex), j++;
            world.scoreboard.getObjective(this.fullName).setScore(`${k}`, 0)
        }
        return this;
    }
    /**
    ...
    delete(key: string): this {
        ...
        for (let l = 1; l < length; l++) world.scoreboard.getObjective(this.fullName).removeParticipant(`${key}=${l}`)
        world.scoreboard.getObjective(this.fullName).removeParticipant(key)
        return this;
    }
    /**
     * Delete the key from the table
    ...
    deleteMany(keys: string[]): this {
        ...
        for (const k of keys) {
            ...
            for (let l = 1; l < length; l++) try { world.getDimension('overworld').runCommand(`scoreboard players reset "${k}=${l}" "${this.fullName}"`); } catch { }
            try { world.getDimension('overworld').runCommand(`scoreboard players reset "${k}" "${this.fullName}"`); } catch { }
        }
        return this;
    }
    /**
     ...
     */
    clear(): this {
        try { world.scoreboard.removeObjective(this.fullName) } catch { };
        try { world.scoreboard.addObjective(this.fullName, this.fullName) } catch { };
        return this;
    }
    /**
