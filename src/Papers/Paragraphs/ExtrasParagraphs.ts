...
export const
     ...
     * @returns {boolean}
     */
    betweenXYZ = (XYZa: number[], XYZb: number[], XYZc: number[]) => XYZc.every((c: number, i: number) => (c >= Math.min(XYZa[i], XYZb[i])) && (c <= Math.max(XYZa[i], XYZb[i]))),
    /**
     ...
     */
    isReach = (pos1: Vector3, pos2: Vector3) => Math.sqrt((pos1.x - pos2.x) ** 2 + (pos1.y - pos2.y) ** 2 + (pos1.z - pos2.z) ** 2) > 7,
    /**
     * GET RANDOM DIGIT ID
     */
 ...
 */
export async function confirmForm(player: Player | PlayerType, title: string, body: string, yes?: string, no?: string): Promise<boolean> {
    let confirm = false;
 ...
 */
export async function sleep(ticks: number, callback?: () => void): Promise<void> {
...
export function compass(angle: number): string {
    let face = '';
    if (Math.abs(angle) > 112.5) face += 'North ';
    if (Math.abs(angle) < 67.5) face += 'South ';
    if (angle < 157.5 && angle > 22.5) face += 'West';
    if (angle > -157.5 && angle < -22.5) face += 'East';
    return face.trim();
}
 ...
 * @returns {string}
 */
export function grammarText(text: string): string {
    try {
        text = text.match(/:([\s\S]*)$/)[1].replace(/[\W_]/g, ' ').split(' ').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
        return text;
    } catch {
        return text
    }
...
export function pokemonText(name: string): string {
    try {
        if (!name.includes(':')) name = `pokeworld:${name}`;
        if (!name.includes(':wild_')) name = name.toLowerCase().replace(':', ':wild_').replace(/\s/g, '_');
        return name;
    } catch {
        return new Error(`Unable to pokemon text "${name}"`) as unknown as string;
    }
}
