import { Player, world, system, Entity, EntityEquippableComponent, EquipmentSlot } from "@minecraft/server";

// 🕳️ Cave Pokémon Groups (Gen 1–4 only)
const common = [
  'wild_diglett', 'wild_geodude', 'wild_gastly', 'wild_zubat'];

const uncommon = [
  'wild_graveler', 'wild_dugtrio', 'wild_golbat', 'wild_wartortle',
  'wild_kadabra', 'wild_sableye', 'wild_misdreavus'];

const rare = [
  'wild_gengar', 'wild_bronzong', 'wild_claydol', 'wild_magnezone',
  'wild_mismagius', 'wild_spiritomb'];

const veryRare = [
  'wild_shuckle', 'wild_gliscor', 'wild_lunatone', 'wild_solrock',
  'wild_dusknoir', 'wild_probopass'];

const ultraRare = [
  'wild_garchomp', 'wild_metagross', 'wild_tyranitar'
];
...
system.runInterval(() => {
    for (const player of world.getPlayers()) {
        ...
        const inPlains = getScore(player, "cave");
