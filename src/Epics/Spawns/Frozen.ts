import { Player, world, system, Entity, Vector3, EquipmentSlot, EntityEquippableComponent } from "@minecraft/server";

// ❄️ Frozen Biome Pokémon Rarity Groups (Finalized)
const common = [
  'wild_swinub','wild_snorunt',
  'wild_spheal','wild_sneasel','wild_snover','wild_delibird'
];

const uncommon = [
  'wild_piloswine','wild_glalie',
  'wild_sealeo','wild_jynx','wild_smoochum'
];

const rare = [
  'wild_walrein','wild_abomasnow', 'wild_froslass','wild_weavile'];

const veryRare = [
  'wild_mamoswine','wild_glaceon'];
...
system.runInterval(() => {
    for (const player of world.getPlayers()) {
        ...
        const inPlains = getScore(player, "Frozen");
